...
- broken Chinese continuity: related claims stay in one sentence, joined by commas / 而 / 所以. Do not copy English period-stacking. Split only when the thought turns. 「原生自回归是两件事，预训练拟合 p，写出下一步时网络只跑一次」is one breath; 「是两件事。预训练拟合 p。写出下一步，网络只跑一次。」is not. 「写出下一步网络只跑一次」is jammed; write 写出下一步时网络只跑一次.
