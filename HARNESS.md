...
3. **Leftover-AI pass.** Search for: 只有…才, 不是 A 而是 B, 因子, 借口, 回路, 门口, 本文, 第一组, 第二组, 实验里, 该赢, 否定原生, 还能做的，是, 口头上的, 要测的是, 在那上面是对的, 不要…一遍, *first experiment*, *second experiment*, *supposed to win*, *what remains is*, *the test now is*, *collapses to*, *what to learn is not*, *misses both ends*, *the intuition is not hard*. A design sentence that ends in 不要 X is usually 不用 / 不必: the point is that the extra step is unnecessary, not a prohibition.
