...
\(c_t\) 是当时已经有的条件：文本、第一帧、相机位姿、按键。它可以进条件，但不等于可以去看还没发生的 \(v_{>t}\)。这里说的原生自回归有两层意思：预训练拟合的就是这个乘积；写出下一步，网络只跑一次。Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) 是我们按现在通行的扩散自回归训过的模型之一。下面说的是这套做法，以及它离这个定义还有多远。
...
更晚的后训练，采样和窗口固定，同一块里纹理更干净，跨块的时间一致性并没有稳定跟着变好。这类后训练优化的是块内目标。时间乘积要的，是块与块之间的条件。

固定生成器、只改 \(\mathcal{H}_i\)——加长推理窗口，打开或关掉位置、位姿编码的重置——两者都不改训练条件。窗口和训练对不上，偏差出在块与块的接缝；重置和训练对不上，相对位置编码会指向另一套几何。跨块一致性没有因此稳定变好。变的仍是测试时看见谁，不是把 \(q_{\mathrm{bi}}\) 换成 \(p(v_t\mid v_{<t})\)。

---

## 对原生设计意味着什么

块内后训练，以及推理时改窗口、开关重置，都到不了 \(p(v_t\mid v_{<t})\)。如果预训练就要拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，下一步又只跑一次网络，后面几件事其实已经定了。

**块内后训练补不上跨块条件。** 块内损失会把能力花在 \(B_i\) 好不好看上。要把 \(\prod_t p(v_t\mid v_{<t})\) 学到，预训练的条件就得是过去。再加一个更好看的蒸馏阶段，补不上这个条件。继续预训练一条已经双向的 DiT，初始化可以留，但时间上的未来必须从第一步起就看不见。更干净的做法是同一套隐空间、帧内仍双向，时间 mask 从随机初始化就是因果的。VideoGPT [7] 写过这个分解，离散码本偏弱；缺的是把它落到 DiT 和现在的视频 VAE 上 [12][15][16]。损失可以还是流匹配，改的是条件：
...
**加长窗口、开关重置，改的不是训练条件。** 只改 \(\mathcal{H}_i\)、不改训练条件，跨块一致性没有稳定变好。所以在原生模型里，看得见的过去应当写进模型结构：要么就是 \(v_{<t}\)（因果注意力的缓存），要么是参数里对 \(v_{<t}\) 的压缩，而且梯度能从这段历史传回去。固定「留多少锚点、留多少最近帧」，再去搜要不要重置位置编码，只在把 \(q_{\mathrm{bi}}\) 改造成 \(q_{\mathrm{AR}}\) 时才有必要。FramePack [26]、WorldMem [27] 问的是这段历史里该留什么。答案不该是再扫一轮和训练对不齐的窗口。

**时间单元在预训练定下来，测试时不再对块内积分。** 块内去噪可以更好看，学生的能力却还绑在 \(K\) 步向量场上。如果说一块（或一帧）就是一步，测试时就不能再对这块做 \(K\) 步积分。一次网络计算写一帧，最接近语言里的一个词；一次网络计算写 \(L>1\) 帧，块内可以双向，每次吐出的片段更长。两种都可以，但 \(L\) 不能再当推理超参：改 \(L\) 就是改这一步有多长。块长如果还留给测试时折中，就会回到三帧加一段微分方程，后训练会再去优化这段方程。

**评价要看跨块，不能只看块内干不干净。** 块内更干净和跨块更一致是两回事。如果用短片段、块内指标决定何时停训，还会再走到这条分叉上。应当用同一套权重、同一种只跑一次网络的生成方式，从第一秒量到超过训练窗口的长度；\(c_t\) 中途变了，再看条件是不是仍只依赖 \(v_{<t}\)。双向模型在十秒片段上继续更好看，本就是 \(q_{\mathrm{bi}}\) 该赢的地方，不能拿这个否定原生。

**数据也要按「过去已经发生」来切。** 等长的短片段把未来帧当成白送的特征。\(q_{\mathrm{AR}}\) 要求数据里的过去也是已经发生的过去：更长的连续镜头、带时间戳的 \(c_t\)、中途才出现的条件。不改训练条件，跨块一致性上不来。只改 \(\mathcal{H}_i\) 的时候，我们见过这件事。
