...
2021 年离开 VideoGPT 这条时间分解，当时说得通，那时候要的是短片段，短片段上 \(q_{\mathrm{bi}}\) 就是对的模型。现在要测的变成了流，先训双向短片，再做后训练，再在推理时搜 \(\mathcal{H}_i\)，到不了 \(p(v_t\mid v_{<t})\)，接下来只能在预训练改训练时能看见的变量。没改之前，现在叫「自回归视频」的做法仍是先按整段短片去噪，再在推理时改成只看过去。
