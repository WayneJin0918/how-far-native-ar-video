...
\(c_t\) 是当时已经有的东西：文本、第一帧、相机位姿、按键。它可以进条件，但不能拿它当借口去看还没发生的 \(v_{>t}\)。我后面说的原生自回归，就是两件事：预训练拟合的是这个乘积；写出下一步，网络只跑一次。Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) 是我们按现在通行的扩散自回归训出来的。下面拿它当例子，看看现在这套做法离这个定义还有多远。
