...
2021 年离开 VideoGPT 这条时间分解，当时说得通：要的是短片段，\(q_{\mathrm{bi}}\) 在那上面是对的。现在要测的是流。双向短片之后再做后训练，再在推理时搜 \(\mathcal{H}_i\)，到不了 \(p(v_t\mid v_{<t})\)。还能做的，是预训练就改训练时能看见的变量。在那之前，口头上的「自回归视频」仍然是：先按整段短片去噪，再在推理时改成只看过去。
