...
语言模型里，自回归是什么，大家其实早就说清楚了。预训练拟合的是

\[
p(w_{1:n})=\prod_i p(w_i\mid w_{<i}),
\]

每个因子算一次。训练看见的、测试看见的，都是已经写出的词。没有人会先训一个双向 BERT [6]，再蒸馏出一个「看起来像 GPT」的学生，然后管它叫原生自回归。推理时加一层因果 mask，也不算。

视频如果也认真用这个词，就应该写成：对 \(V=(v_1,\ldots,v_T)\)，

\[
p(V\mid c)=\prod_t p(v_t\mid v_{<t},c_{\le t}).
\]

\(c_t\) 是当时已经有的条件：文本、第一帧、相机位姿、按键。它进条件，但不等于可以去看还没发生的 \(v_{t+12}\)。预训练就拟合这个乘积、一次前向写出其中一个因子，我们才叫它原生自回归。Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) 是我们按现在通行的扩散自回归训出来的一条。后面拿它当例子，看现在的做法离这个定义有多远。

---

## 图像没有时间；视频多出来的是 \(t\)

一张图里，左右同时在。PixelRNN [4] 把似然写成光栅顺序 \(p(x)=\prod_{h,w}p(x_{hw}\mid x_{<hw})\)，只是为了给联合分布排个序。这个序是编码约定。DiT [12] 把它拿掉，在 \((h,w)\) 上双向，更像一张照片。一帧里面空间双向，并不犯规。

视频多了 \(t\)。Video Pixel Networks [5] 在像素级就已经把 \(t\) 当序列，而不是把时长 \(T\) 拉成一张更高的图。写 \(v_t\) 的时候，\(v_{t+1}\) 还不存在——不是「还没画上」，是根本还没发生。所以同一个似然里，两种顺序的地位不一样：
...
左边这个乘积来自时间：后面的帧，这一刻还不存在。右边帧内双向，只是怎么表示。把 \(T\) 当成更长的 \(H\)，等于把「还没发生」当成「画面另一边还没画完」。短短几秒的片子上，这个差别几乎看不出来：未来帧就在同一条训练样本里，用上它们画质更好。Sora [14]、Wan [16]、LTX [15] 赢的就是这一块。它们拟合整段短片的联合分布，所以每个时刻的条件是

\[
q_{\mathrm{bi}}(v_t\mid V_{\setminus t},c).
\]

可一旦测试变成流——接着往下写、\(c_t\) 现场到、生成比训练时见过的窗更长——真正需要的是

\[
q_{\mathrm{AR}}(v_t\mid v_{<t},c_{\le t})=p_{\mathrm{data}}(v_t\mid v_{<t},c_{\le t}).
\]

训练时能看见什么，学到的函数就不一样。\(c_t\) 换成文本、图像或位姿，只是右边多一个条件，并不会把左边的 \(V_{\setminus t}\) 变成 \(v_{<t}\)。世界模型 [22]–[25] 很早就把这个错位露出来了，因为人还在里面玩；更长的视频、更低延迟的续写，碰到的是同一件事。

后来走的路，其实可以不这么走。VideoGPT [7]、TATS [9] 已经在离散符号上写过 \(\prod_t p(v_t\mid v_{<t})\)。画质输给 MaskGIT [8]、MAGVIT [11]、Phenaki [10]，再输给按整段短片训的 DiT 级双向模型。图好看了，时间上的分解却丢掉了。产品变成流之后，领域没有回到「时间上只看过去」的预训练，而是在问：已经训好的 \(q_{\mathrm{bi}}\)，怎么改一改，让测试看起来像 \(q_{\mathrm{AR}}\)。后面整条流水线，都是在答这道题。

---

## 训练看见了未来，一次前向也写不出一个因子

原生要两件事同时成立：训练时条件只能看见已经发生的过去；一个因子就是一次网络计算。现在通行的扩散自回归，两件都不成立。

**训练看见的并不只有过去。** 训练本该是 \(q_{\mathrm{train}}(v_t\mid\cdot)=p_{\mathrm{data}}(v_t\mid v_{<t})\)。如果拟合的是 \(q_{\mathrm{bi}}\)，推理时再加因果 mask（当前块不许看后面的块），只是测试时把训练用过的变量删掉。模型从来没有在「只有过去」这个条件下出过梯度。
...
真正在生成的是一段常微分方程，不是 \(B_i\)。\(K\sim 30\) 是短片双向模型的积分长度；\(K=4\) 只是把同一种向量场的步数缩短，并没有变成 \(f_\theta(v_{<t})\)。

训练已经看见未来，推理就只能先戴上 mask，训练再用干净的过去做 teacher forcing——「只有过去」又贵又不稳：
...
左边是数据，右边是模型自己刚写出的块。Bengio 等的 scheduled sampling [2]、Ranzato 等的序列级训练 [3]、Self Forcing [19]，说的都是这个缺口。缺口还在，就会再加一级：让模型自己往下滚，在这段历史上做 DMD [28]，再把损失加到已经带误差的历史上，\(\mathcal{L}_{\mathrm{long}}=\sum_k\mathcal{L}_{\mathrm{DMD}}^{(k)}\)。预训练从没在「只有过去」下出过梯度，这些阶段都是事后补的。
...
训练条件没改，因子还是多步积分，生成又要比训练窗更长，就只能改推理时看得见的过去 \(\mathcal{H}_i\)：留几帧早期的当锚点、只留最近几块、窗口滑走时重置位置编码。StreamingLLM [13] 在语言里说过，滑动窗口需要锚点；FramePack [26]、WorldMem [27] 问的是 \(\mathcal{H}_i\) 里该留什么。这些改的是测试时看见谁，不是训练时拟合的是哪一个条件。
...
每一级都多一组要调的量。如果预训练从来没用过 \(v_{>t}\)，并且在说好的时间单元上一次前向就写出一个因子，这些级根本不必存在。

块该切多长，也是被逼出来的折中，不是可以随便选的。一块一帧，剩下的微分方程没有块内运动可积；一块接近一秒，又变回带边界缓存的短片模型。三帧既能边写边出，又给块内双向去噪留了空间，所以成了常见折中。原生模型得在预训练就定下这个单元——一帧，或一块一次前向——而不是对共享 mask 的三帧再积分一遍。

---

## 补充实验

我们在 Echo-WM-Flash [1] 上另外做了两组闭环、长程一致性实验，论文公开表里没有。一组换后训练阶段，一组只改推理时看得见的过去 \(\mathcal{H}_i\)。

后训练如果优化的是块内去噪，同一块 \(B_i\) 里可以更好看，但不一定能改善 \(\prod_t p(v_t\mid v_{<t})\) 要的、块与块之间的条件。只改 \(\mathcal{H}_i\)，改的是测试时看见谁，不是训练时拟合的条件。

第一组固定采样和窗口，比较不同后训练阶段的生成器。更晚的检查点，同一块 \(B_i\) 里纹理更干净；跨块的时间一致性并没有稳定跟着变好。我们做的这类后训练，优化的是块内目标。乘积要的条件在块与块之间。
...
Flash 上看到的，和前面的分析是一件事：块内后训练，以及推理时改窗口、开关重置，都到不了 \(p(v_t\mid v_{<t})\)。如果预训练就要拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，并在选定的时间单元上一次前向写出这个因子，下面几件事是跟着来的。

**块内后训练补不上跨块条件。** Flash 上更晚的后训练已经说明：块内损失会把能力花在 \(B_i\) 好不好看上。要把 \(\prod_t p(v_t\mid v_{<t})\) 学到，预训练的条件就得是过去，而不是再加一个更好看的蒸馏阶段。继续预训练一条已经双向的 DiT，可以留下初始化，但时间上的未来必须从第一步起就看不见。更干净的做法是同一套隐空间、帧内仍双向，时间 mask 从随机初始化就是因果的。VideoGPT [7] 写过这个分解，离散码本偏弱；缺的是把它落到 DiT 和现在的视频 VAE 上 [12][15][16]。损失可以还是流匹配，改的是条件：
...
\(\sigma\) 只加在当前单元上，梯度不经过 \(v_{>t}\)。少步一致性训练如果要当 \(K=1\) 的手段，必须一开始就在 \(q_{\mathrm{AR}}\) 下训。先学 \(q_{\mathrm{bi}}\) 再压步数，能力又会花在块内积分上——第一组实验里，后训练擅长的正是这个，跨块一致性却带不过去。自己滚一段再加 DMD，还可以继续补这个缺口，但那是上一节写过的补丁，不是原生。

**加长窗口、开关重置，改的不是训练条件。** 第二组实验改了 \(\mathcal{H}_i\)，没改训练条件，跨块一致性也没有稳定变好。所以在原生里，看得见的过去应当是结构：要么就是 \(v_{<t}\)（因果注意力的缓存），要么是参数里对 \(v_{<t}\) 的压缩，而且梯度能从这段历史传回去。固定「留多少锚点、留多少最近帧」、再去搜要不要重置位置编码，只在把 \(q_{\mathrm{bi}}\) 改造成 \(q_{\mathrm{AR}}\) 时才有必要。FramePack [26]、WorldMem [27] 问的是这段历史里该留什么；答案不该是再扫一轮和训练对不齐的窗口。

**时间单元在预训练定下来，测试时不再对块内积分。** 第一组实验改善的是块内去噪器的输出，说明现在这个学生的能力还绑在 \(K\) 步向量场上。原生如果说一块（或一帧）是一个因子，测试时就不能再对这块做 \(K\) 步积分。一次前向写一帧，最接近语言里的一个词；一次前向写 \(L>1\) 帧，块内可以双向，每次吐出的更长。两种都可以，但 \(L\) 不能再当推理超参：改 \(L\) 就是改因子。块长如果还留给测试时折中，就会回到三帧加一段微分方程，后训练会再去优化这段方程。

**评价要看跨块，不能只看块内干不干净。** 第一组实验里，块内更干净和跨块更一致是两回事。如果用短片段、块内指标决定何时停训，还会再走到这条分叉上。应当用同一套权重、同一种一次前向，从第一秒量到超过训练窗的长度，并在 \(c_t\) 中途改变时，看条件是不是仍只依赖 \(v_{<t}\)。双向模型在十秒片段上继续更好看，本就是 \(q_{\mathrm{bi}}\) 该赢的地方，不能拿这个否定原生。

**数据也要按「过去已经发生」来切。** 等长的短片段把未来帧当成白送的特征。\(q_{\mathrm{AR}}\) 要求数据里的过去也是已经发生的过去：更长的连续镜头、带时间戳的 \(c_t\)、中途才出现的条件。不改训练条件，跨块一致性上不来，第二组实验已经见过。

还有一些问题没关上，而且值得在再调窗口之前先想：已经双向的大规模视频骨干，只做时间因果的继续预训练，要多久才能去掉 \(q_{\mathrm{bi}}\) 留下的习惯，要不要重新初始化？连续隐空间上 \(K=1\) 稳不稳，还是需要更强的离散或混合码本？原生目标下，一帧一次前向和三帧一次前向，哪一个更能同时保住运动和延迟？学出来的 \(\mathcal{H}_i\) 要不要直接接收跨块一致性的信号——第二组实验说明，没有这条信号的窗口搜索，并不改变分解。

2021 年离开这条路，说得通：那时要的是短片段，\(q_{\mathrm{bi}}\) 在那上面是对的。现在要测的是流。Flash [1] 上我们试过的后训练，以及推理时对 \(\mathcal{H}_i\) 的搜索，到不了 \(p(v_t\mid v_{<t})\)。还能走的一步，是预训练就改训练时能看见的变量。在那之前，「自回归视频」仍然是：先按整段短片去噪，再在推理时改成只看过去。
