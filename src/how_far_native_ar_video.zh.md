...
语言模型里，自回归早就不是个含糊的词。预训练拟合的就是

\[
p(w_{1:n})=\prod_i p(w_i\mid w_{<i}),
\]

下一步出下一个词，网络只跑一次。训练看见的、测试看见的，都是已经写出的词。没人会先训一个双向 BERT [6]，再蒸馏出一个「看起来像 GPT」的学生，然后说这就是原生自回归。推理时加一层因果 mask，也算不上。

视频要是也想用这个词，写法其实很直接。对 \(V=(v_1,\ldots,v_T)\)，

\[
p(V\mid c)=\prod_t p(v_t\mid v_{<t},c_{\le t}).
\]

\(c_t\) 是当时已经有的东西：文本、第一帧、相机位姿、按键。它可以进条件，但不能拿它当借口去看还没发生的 \(v_{t+12}\)。我后面说的原生自回归，就是两件事：预训练拟合的是这个乘积；写出下一步，网络只跑一次。Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) 是我们按现在通行的扩散自回归训出来的。下面拿它当例子，看看现在这套做法离这个定义还有多远。

---

## 图像没有时间；视频多出来的是 \(t\)

一张图里，左右是同时在的。PixelRNN [4] 把似然写成光栅顺序 \(p(x)=\prod_{h,w}p(x_{hw}\mid x_{<hw})\)，不过是给联合分布排个序。这个序是编码约定，跟时间没关系。DiT [12] 后来把光栅顺序丢掉，在 \((h,w)\) 上做双向注意力，反而更像一张照片。一帧里面空间双向，没什么问题。

视频多出来的是 \(t\)。Video Pixel Networks [5] 在像素级就已经把 \(t\) 当序列，而不是把时长 \(T\) 拉成一张更高的图。写 \(v_t\) 的时候，\(v_{t+1}\) 还不存在。不是「还没画上」，是根本还没发生。所以同一个似然里，两种顺序的地位本来就不一样：
...
左边这个乘积来自时间：后面的帧这一刻还不存在。右边帧内双向，只是当前帧怎么表示。要是把 \(T\) 当成更长的 \(H\)，等于把「还没发生」当成「画面另一边还没画完」。短短几秒的片子上，这个差别几乎看不出来——未来帧就在同一条训练样本里，用上它们画质更好。Sora [14]、Wan [16]、LTX [15] 赢的就是这一块。它们拟合的是整段短片的联合分布，所以每个时刻的条件其实是

\[
q_{\mathrm{bi}}(v_t\mid V_{\setminus t},c).
\]

可测试一旦变成流，事情就不一样了。接着往下写、\(c_t\) 现场到、生成比训练时见过的窗口更长，这时真正需要的是

\[
q_{\mathrm{AR}}(v_t\mid v_{<t},c_{\le t})=p_{\mathrm{data}}(v_t\mid v_{<t},c_{\le t}).
\]

训练时能看见哪些变量，学到的函数就不一样。\(c_t\) 换成文本、图像还是位姿，只是条件里多一项，\(V_{\setminus t}\) 不会因此变成 \(v_{<t}\)。世界模型 [22]–[25] 很早就把这个错位露出来了，人还在里面交互；更长的视频、更低延迟的续写，碰到的是同一件事。

后来走的路，其实可以不这么走。VideoGPT [7]、TATS [9] 已经在离散符号上写过 \(\prod_t p(v_t\mid v_{<t})\)。画质后来输给 MaskGIT [8]、MAGVIT [11]、Phenaki [10]，再输给按整段短片训的 DiT 级双向模型。图是好看了，时间上的分解却丢掉了。产品变成流之后，领域没有回到「时间上只看过去」的预训练，而是在问：已经训好的 \(q_{\mathrm{bi}}\)，怎么改一改，让测试看起来像 \(q_{\mathrm{AR}}\)。后面整条流水线，都是在答这道题。

---

## 训练看见了未来，生成一步也不是一次网络计算

按上面那个定义，现在通行的扩散自回归两头都不沾。训练时条件看见了未来；纸上写的每一步，也不是只跑一次网络。

先说训练。本该是 \(q_{\mathrm{train}}(v_t\mid\cdot)=p_{\mathrm{data}}(v_t\mid v_{<t})\)。如果拟合的是 \(q_{\mathrm{bi}}\)，推理时再加因果 mask（当前块不许看后面的块），只是测试时把训练用过的变量删掉。模型从来没有在「只有过去」这个条件下回传过梯度。

再说生成。时间切成 \(B_1,\ldots,B_N\) 之后，纸上写的是 \(p(B_{1:N})=\prod_i p_\theta(B_i\mid B_{<i})\)。可这一步如果做成
...
真正在生成的并不是 \(B_i\)，而是一段常微分方程。\(K\sim 30\) 是短片双向模型的积分长度；\(K=4\) 只是把同一种向量场的步数缩短，并没有变成 \(f_\theta(v_{<t})\)。

于是补丁就一层层长出来。训练已经看见未来，推理就只能先戴上 mask。训练时历史用的是干净数据（teacher forcing），测试时历史却是模型自己刚写出的块：
...
左边是数据，右边是模型自己刚写的。Bengio 等的 scheduled sampling [2]、Ranzato 等的序列级训练 [3]、Self Forcing [19]，说的都是这个缺口。真要在「只有过去」下训，又贵又不稳。缺口还在，就再加一级：先让模型自己往下滚，在这段历史上做 DMD [28]；再把同样的损失加到已经带误差的更长历史上，\(\mathcal{L}_{\mathrm{long}}=\sum_k\mathcal{L}_{\mathrm{DMD}}^{(k)}\)。预训练从没在「只有过去」下回传过梯度，这些阶段都是事后补的。
...
训练条件没改，每一步仍是多步积分。生成又要比训练窗口更长，就只能改推理时看得见的过去 \(\mathcal{H}_i\)：留几帧早期的当锚点、只留最近几块、窗口滑走时重置位置编码。StreamingLLM [13] 在语言里说过，滑动窗口需要锚点；FramePack [26]、WorldMem [27] 问的是 \(\mathcal{H}_i\) 里该留什么。这些改的是测试时看见谁，不是训练时拟合的是哪一个条件。

现在通行的做法，就是缺口露出来一层、补一层：
...
每一级都多一组要调的量。如果预训练从来没用过 \(v_{>t}\)，下一步又只跑一次网络，这些级根本不必存在。

块该切多长，也是被逼出来的，不是可以随便选的。一块只有一帧，块内没有时间上的运动可积，多步积分只剩空间去噪；一块接近一秒，又变回带边界缓存的短片模型。三帧既能边写边出，又给块内双向去噪留了空间，所以成了常见折中。原生模型得在预训练就定下这个单元：一帧跑一次网络，或者一块跑一次网络，而不是对共享 mask 的三帧再积分一遍。
...
直觉上也不难想。后训练如果优化的是块内去噪，同一块 \(B_i\) 里可以更好看，但不一定能改善块与块之间的条件；而 \(\prod_t p(v_t\mid v_{<t})\) 要的正是后者。只改 \(\mathcal{H}_i\)，改的是测试时看见谁，不是训练时拟合的条件。

第一组固定采样和窗口，比较不同后训练阶段的生成器。更晚的检查点，同一块 \(B_i\) 里纹理更干净，跨块的时间一致性并没有稳定跟着变好。我们做的这类后训练，优化的就是块内目标。时间上的乘积要的条件，在块与块之间。

第二组固定生成器，只改 \(\mathcal{H}_i\)：加长推理窗口，以及打开或关掉位置、位姿编码的重置。两者都不改训练条件。窗口和训练对不上，偏差出在块与块的接缝；重置和训练对不上，相对位置编码会指向另一套几何。跨块一致性没有因此稳定变好。改来改去，还是测试时看见谁，不是把 \(q_{\mathrm{bi}}\) 换成 \(p(v_t\mid v_{<t})\)。

---

## 对原生设计意味着什么

Flash 上看到的，和前面的分析对得上：块内后训练，以及推理时改窗口、开关重置，都到不了 \(p(v_t\mid v_{<t})\)。如果预训练就要拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，下一步又只跑一次网络，后面几件事其实已经定了。
...
\(\sigma\) 只加在当前单元上，梯度不经过 \(v_{>t}\)。少步一致性训练如果想当 \(K=1\) 的手段，得一开始就在 \(q_{\mathrm{AR}}\) 下训。先学 \(q_{\mathrm{bi}}\) 再压步数，能力又会花在块内积分上。第一组实验里，后训练擅长的正是这个，跨块一致性却没有跟着过来。自己滚一段再加 DMD，还可以继续补，但那是上一节写过的补丁，不是原生。

**加长窗口、开关重置，改的不是训练条件。** 第二组实验改了 \(\mathcal{H}_i\)，没改训练条件，跨块一致性也没有稳定变好。所以在原生模型里，看得见的过去应当进模型：要么就是 \(v_{<t}\)（因果注意力的缓存），要么是参数里对 \(v_{<t}\) 的压缩，而且梯度能从这段历史传回去。固定「留多少锚点、留多少最近帧」，再去搜要不要重置位置编码，只在把 \(q_{\mathrm{bi}}\) 改造成 \(q_{\mathrm{AR}}\) 时才有必要。FramePack [26]、WorldMem [27] 问的是这段历史里该留什么；答案不该是再扫一轮和训练对不齐的窗口。
...
还有几个问题没有答案，而且最好在再调窗口之前先想清楚。已经双向的大规模视频骨干，只做时间因果的继续预训练，要多久才能去掉 \(q_{\mathrm{bi}}\) 留下的习惯？要不要重新初始化？连续隐空间上 \(K=1\) 稳不稳，还是需要更强的离散或混合码本？原生目标下，一帧跑一次网络和三帧跑一次网络，哪一个更能同时保住运动和延迟？学出来的 \(\mathcal{H}_i\) 要不要直接接收跨块一致性的信号？第二组实验说明，没有这条信号的窗口搜索，并不改变模型学到的分解。

2021 年离开 VideoGPT 这条时间分解，当时说得通：要的是短片段，\(q_{\mathrm{bi}}\) 在那上面是对的。现在要测的是流。Flash [1] 上我们试过的后训练，以及推理时对 \(\mathcal{H}_i\) 的搜索，到不了 \(p(v_t\mid v_{<t})\)。还能走的一步，是预训练就改训练时能看见的变量。在那之前，大家口头上的「自回归视频」，仍然是先按整段短片去噪，再在推理时改成只看过去。

---

## 结论

说到底就两件事。预训练拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，写出下一步只跑一次网络。现在通行的做法两头都不成立：训练看见了未来，生成一步仍是一段微分方程。Flash 上的两组实验只是把这件事对了一遍——块内后训练让同一块更好看，改窗口、开关重置只改测试时看见谁，跨块一致性都没有稳定跟着变好。补丁可以让推理看起来像自回归，到不了那个乘积。

我们离这条路其实还很远。远的不是再缺一个更好的蒸馏阶段，也不是窗口还没搜完，而是预训练看见的变量从一开始就不是 \(v_{<t}\)。语言这边，这件事在预训练阶段就已经钉死了，写的就是 \(\prod_i p(w_i\mid w_{<i})\)。后面的长上下文、工具、推理，都是长在这个乘积上的。视频 2021 年离开了同一条分解，换短片上的画质，赢了几年。产品变成流之后，领域没有回到那个乘积，而是用因果 mask、少步积分和窗口搜索去模仿它。

语言模型这几年走得很快，并不是因为它先把下一句说圆，再在推理时改成只看过去。它是一开始就按下一词来训，再把规模和数据堆上去。视频要是还把「自回归」用在一条先双向、再因果化的流水线上，和语言的距离只会越拉越大。该跟的不是语言的某一层模块，是这条路：预训练只看已经发生的过去，下一步只跑一次网络。现在还早。
