...
视频如果也用这个词，对应的写法是：对 \(V=(v_1,\ldots,v_T)\)，

\[
p(V\mid c)=\prod_t p(v_t\mid v_{<t},c_{\le t}).
\]

\(c_t\) 是当时已经有的条件：文本、第一帧、相机位姿、按键。它可以进条件，但不等于可以去看还没发生的 \(v_{>t}\)。本文说的原生自回归有两层意思：预训练拟合的就是这个乘积；写出下一步，网络只跑一次。Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) 是我们按现在通行的扩散自回归训出来的。下面用它看这套做法离这个定义还有多远。

---

## 图像没有时间；视频多出来的是 \(t\)

一张图里，左右是同时在的。PixelRNN [4] 把似然写成光栅顺序 \(p(x)=\prod_{h,w}p(x_{hw}\mid x_{<hw})\)，只是给联合分布排个序。这个序是编码约定，不是时间。DiT [12] 后来把光栅顺序丢掉，在 \((h,w)\) 上做双向注意力，更接近一张照片。一帧里面空间双向，没有问题。

视频多出来的是 \(t\)。Video Pixel Networks [5] 在像素级就已经把 \(t\) 当序列，而不是把时长 \(T\) 拉成一张更高的图。写 \(v_t\) 的时候，\(v_{t+1}\) 还不存在：不是画面还没画完，是这件事还没发生。所以同一个似然里，两种顺序的地位不一样：
...
测试变成流以后，需要的条件就变了。接着往下写、\(c_t\) 是现场来的、生成比训练时见过的窗口更长，这时要的是

\[
q_{\mathrm{AR}}(v_t\mid v_{<t},c_{\le t})=p_{\mathrm{data}}(v_t\mid v_{<t},c_{\le t}).
\]

训练时能看见哪些变量，学到的函数就不一样。\(c_t\) 换成文本、图像或位姿，只是条件里多一项，\(V_{\setminus t}\) 不会变成 \(v_{<t}\)。世界模型 [22]–[25] 很早就把这个错位露出来了，因为人还在回路里交互。更长的视频、更低延迟的续写，暴露的是同一处错位。
...
先看训练。本该是 \(q_{\mathrm{train}}(v_t\mid\cdot)=p_{\mathrm{data}}(v_t\mid v_{<t})\)。如果拟合的是 \(q_{\mathrm{bi}}\)，推理时再加因果 mask（当前块不许看后面的块），只是测试时把训练用过的变量删掉。模型从来没有在「只有过去」这个条件下回传过梯度。
...
这两条缺口会接着往下长出后训练。训练已经看见未来，推理就只能加上因果 mask。训练时历史用的是干净数据（teacher forcing），测试时历史却是模型自己刚写出的块：
...
左边是数据，右边是模型自己刚写的。Bengio 等的 scheduled sampling [2]、Ranzato 等的序列级训练 [3]、Self Forcing [19]，处理的都是这个缺口。真要在「只有过去」下训，又贵又不稳。缺口还在，就再加一级：先让模型自己往下滚，在这段历史上做 DMD [28]；再把同样的损失加到已经带误差的更长历史上，\(\mathcal{L}_{\mathrm{long}}=\sum_k\mathcal{L}_{\mathrm{DMD}}^{(k)}\)。预训练从没在「只有过去」下回传过梯度，这些阶段都是事后加上的。
...
原生自回归是两件事：预训练拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，写出下一步只跑一次网络。现在通行的做法两条都不成立。Flash 上的两组实验对过一遍：块内后训练让同一块更好看，改窗口和重置只改测试时看见谁，跨块一致性都没有稳定跟着变好。
