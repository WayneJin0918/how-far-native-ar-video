...
**评价要看跨块，不能只看块内干不干净。** 块内更干净，并不等于跨块更一致。只看短片段和块内指标来停训，还会再走到这里。用同一套权重、同一种只跑一次网络的生成方式，从第一秒量到超过训练窗口的长度；\(c_t\) 中途变了，再看条件是不是仍只依赖 \(v_{<t}\)。双向模型在十秒片段上继续更好看，本就是 \(q_{\mathrm{bi}}\) 该赢的地方，不能拿这个否定原生。
