...
VideoGPT [7]、TATS [9] 已经在离散符号上写过 \(\prod_t p(v_t\mid v_{<t})\)。画质后来输给 MaskGIT [8]、MAGVIT [11]、Phenaki [10]，再输给按整段短片训的 DiT 级双向模型。图好看了，时间上的分解却丢掉了。测试变成流之后，领域没有回到「时间上只看过去」的预训练，而是在问：已经训好的 \(q_{\mathrm{bi}}\)，怎么改一改，让测试看起来像 \(q_{\mathrm{AR}}\)。后面的流水线，就是在做这件事。
...
更晚的后训练，采样和窗口都没动，同一块里纹理更干净，跨块的时间一致性并没有稳定跟着变好。这类后训练优化的是块内目标。时间乘积要的，是块与块之间的条件。

生成器不动，只改 \(\mathcal{H}_i\)：推理窗口加长，位置和位姿编码的重置打开或关掉。训练条件都没变。窗口和训练对不上，偏差出在块与块的接缝；重置和训练对不上，相对位置编码会指向另一套几何。跨块一致性没有因此稳定变好。变的仍是测试时看见谁。\(q_{\mathrm{bi}}\) 并没有变成 \(p(v_t\mid v_{<t})\)。
...
**评价要看跨块，不能只看块内干不干净。** 块内更干净，并不等于跨块更一致。只看短片段和块内指标来停训，还会再走到这里。用同一套权重、同一种只跑一次网络的生成方式，从第一秒量到超过训练窗口的长度；\(c_t\) 中途变了，再看条件是不是仍只依赖 \(v_{<t}\)。几秒的短片上，双向模型更好看并不奇怪。那段长度上，训练本来就看得见未来。短片上的观感，说明不了原生走不走得通。
