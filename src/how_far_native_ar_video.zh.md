...
按这个定义，现在通行的扩散自回归两条都不成立。训练看见了未来。纸上写的每一步，也不是只跑一次网络。
...
块该切多长，不能随便定。一块只有一帧，块里没有前后运动，多步积分只剩空间去噪。一块接近一秒，又变回带边界缓存的短片模型。三帧既能边写边出，又给块内双向去噪留了空间，所以成了常见折中。原生模型得在预训练就定下这个单元：一帧跑一次网络，或者一块跑一次网络，而不是三帧共用一个 mask，再积分一遍。
...
更晚的后训练，采样和窗口都没动。同一块里纹理更干净，跨块一致性却没有稳定跟着变好。这类后训练优化的是块内目标。时间乘积要的，是块与块之间的条件。
...
原生自回归是两件事：预训练拟合 \(p(v_t\mid v_{<t},c_{\le t})\)，写出下一步只跑一次网络。现在通行的做法两条都不成立。事后再补也到不了。块内损失让同一块更好看。加长窗口或开关重置，只改测试时看见谁。两者都补不上 \(p(v_t\mid v_{<t})\)。

语言模型这几年走得快，是因为它一开始就按下一词来训，再把规模和数据堆上去。视频 2021 年离开了同一条分解，后来用因果 mask、少步积分和窗口搜索去模仿它。要跟上，得在预训练改训练时能看见的变量。再加一级蒸馏补不上。这件事还没做，我们离原生自回归视频模型还很远。
