...
and each factor is one forward pass. Training and test both see words already written. Nobody pretrains a bidirectional BERT [6], distills a student that “looks like GPT,” and calls the student native autoregression. A causal mask at inference is not enough either.
...
\(c_t\) is a condition that has already occurred — text, a first frame, a camera pose, a key press — not a license to look at \(v_{t+12}\), which has not happened yet. We call a model native autoregressive if pretraining already fits this product and one forward emits one of its factors. Echo-WM-Flash [[1]](https://arxiv.org/abs/2608.23189) is one model we trained in the diffusion-autoregressive stack that is now standard; we use it below to see how far that stack sits from this definition.

---

## An image has no time it must obey; video adds \(t\)

An image has no time it must obey. Left and right are present together. PixelRNN [4] wrote \(p(x)=\prod_{h,w}p(x_{hw}\mid x_{<hw})\) only so the joint would have some order. That order is a coding convention. DiT [12] dropped it; bidirectional attention on \((h,w)\) is closer to a photograph. Keeping space bidirectional inside a frame does not violate the temporal constraint.

Video adds \(t\). Video Pixel Networks [5] already treated \(t\) as a sequence at the pixel level, rather than stretching duration \(T\) into a taller image. When \(v_t\) is generated, \(v_{t+1}\) does not exist — not “not yet painted,” not yet occurred. In one likelihood the two orders therefore have different status:
...
The product on the left comes from time: later frames do not exist yet. Bidirectionality inside a frame is a choice of representation. Treating \(T\) as a longer \(H\) treats “has not happened” as “the other side of the picture is unfinished.” On a clip of a few seconds the difference is almost invisible: future frames sit in the same training example, and using them improves quality. Sora [14], Wan [16], and LTX [15] win that local game. They fit the joint over a whole short clip, so the conditional at each time is

\[
q_{\mathrm{bi}}(v_t\mid V_{\setminus t},c).
\]

Once the test is a stream — continuation, a \(c_t\) that arrives live, a horizon longer than the window seen in training — what is needed is

\[
q_{\mathrm{AR}}(v_t\mid v_{<t},c_{\le t})=p_{\mathrm{data}}(v_t\mid v_{<t},c_{\le t}).
\]

Different variables visible in training mean different functions. Swapping text, an image, or a pose for \(c_t\) changes the extra condition on the right; it does not turn \(V_{\setminus t}\) into \(v_{<t}\). World models [22]–[25] made the mismatch visible early, because a person is still in the loop. Longer video and low-latency continuation expose the same mismatch.

The field then took a path it did not have to take. VideoGPT [7] and TATS [9] already wrote \(\prod_t p(v_t\mid v_{<t})\) over discrete symbols. Quality lost to MaskGIT [8], MAGVIT [11], Phenaki [10], then to bidirectional models trained on short clips at DiT scale. Pictures improved; the temporal factorization was dropped. When the product became a stream, the field did not return to pretraining that sees only the past in time. It asked how to reshape an existing \(q_{\mathrm{bi}}\) so that the test looks like \(q_{\mathrm{AR}}\). The pipeline that followed is answering that question.

---

## Training sees the future, and one forward does not emit a factor

Native needs both: the training condition may see only the past that has already occurred, and each factor is one network evaluation. The diffusion-autoregressive stack now in use has neither.
...
the generative unit is a segment of an ordinary differential equation, not \(B_i\). \(K\sim 30\) is the integration length of the short-clip bidirectional model. \(K=4\) shortens the same vector field; it does not become \(f_\theta(v_{<t})\).

Because training already saw the future, inference has to install a mask, and training uses clean past as teacher forcing — “past only” is expensive and unstable:
...
The left-hand side is data. The right-hand side is the block the model just wrote. Scheduled sampling [2], sequence-level training [3], and Self Forcing [19] name this gap. While the gap remains, another stage appears: DMD [28] on a history the model rolled out itself, then the same loss on a history that already contains error, \(\mathcal{L}_{\mathrm{long}}=\sum_k\mathcal{L}_{\mathrm{DMD}}^{(k)}\). Pretraining never produced a gradient under “past only”; these stages were added afterwards.
...
How long a block should be is itself a forced compromise, not a free choice. One frame: the leftover differential equation has no intra-block motion to integrate. Near one second: a short-clip model with a cache on the cut. Three frames are short enough to emit as you go, and still leave room for bidirectional denoising inside the block, which is why that length became common. A native model has to choose the unit at pretraining — one frame, or one forward per block — rather than integrate again over three frames that share a mask.
...
If post-training optimizes denoising inside a block, it can improve appearance inside the same block \(B_i\) without improving the condition between blocks that \(\prod_t p(v_t\mid v_{<t})\) requires. Changing only \(\mathcal{H}_i\) changes who is visible at test time, not the condition fitted in training.

In the first experiment, sampling and the window are held fixed, and generators from later post-training stages are compared. Later checkpoints have cleaner texture inside the same block \(B_i\). Temporal consistency across blocks does not reliably improve with them. The post-training we ran optimizes an intra-block objective. The product asks for a condition between blocks.
...
What remains open, and worth answering before another window sweep, are choices on the product itself. For a large bidirectional video backbone, how long does continued pretraining that is causal in time take to wash out the habits of \(q_{\mathrm{bi}}\), and is a re-initialization required? Is \(K=1\) stable in a continuous latent space, or is a stronger discrete or hybrid codebook needed? Under a native objective, which of one frame per forward and three frames per forward keeps both motion and latency? Should a learned \(\mathcal{H}_i\) receive a signal from consistency across blocks directly — the second experiment says that searching \(\mathcal{H}_i\) without that signal does not change the factorization.

Leaving this path in 2021 was rational: the product then was a short clip, and \(q_{\mathrm{bi}}\) is the right model there. The test now is a stream. The post-training we ran on Flash [1], and the search over \(\mathcal{H}_i\) at inference, do not reach \(p(v_t\mid v_{<t})\). The remaining step is to change, at pretraining, which variables training may see. Until then, “autoregressive video” is still a model that denoises a whole short clip and is only later made to see the past.
